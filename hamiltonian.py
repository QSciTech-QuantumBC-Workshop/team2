...
class OneBodyFermionicHamiltonian(FermionicHamiltonian):
    ...
    def to_linear_combinaison_pauli_string(self,
        ...
        kk = 0
        for ii,ap in enumerate(creation_operators):
            for jj,am in enumerate(annihilation_operators):
                lcps_temp = (ap * am)
                new_coefs[kk]   = self.integrals[ii,jj]*lcps_temp.coefs[0]
                new_coefs[kk+1] = self.integrals[ii,jj]*lcps_temp.coefs[1]
                new_coefs[kk+2] = self.integrals[ii,jj]*lcps_temp.coefs[2]
                new_coefs[kk+3] = self.integrals[ii,jj]*lcps_temp.coefs[3]
                new_pauli_strings[kk]   = lcps_temp.pauli_strings[0]
                new_pauli_strings[kk+1] = lcps_temp.pauli_strings[1]
                new_pauli_strings[kk+2] = lcps_temp.pauli_strings[2]
                new_pauli_strings[kk+3] = lcps_temp.pauli_strings[3]
                kk=kk+4
